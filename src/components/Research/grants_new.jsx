...
const Grants = () => {
  ...
  const fetchGrants = async (endpoint = 'grants') => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '/proxy-backend'}/research/api/${endpoint}/`);
  ...
  const handleSubmitApplication = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '/proxy-backend'}/research/api/grant-applications/`, {
        method: 'POST',
        body: formDataToSend,
      });

...
const Laboratories = () => {
  ...
  useEffect(() => {
    const fetchLabs = async () => {
      try {
        setLoading(true);
        const data = await getLaboratories(lang);
        setLaboratories(Array.isArray(data) ? data : (data?.results || []));
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {laboratories.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
            <Microscope className="w-14 h-14 text-slate-300 mx-auto mb-4" />
            <p className="text-slate-500">{t('common.error', 'Нет данных')}</p>
          </div>
        ) : (
            ...
            {laboratories.map((lab) => (
              <div key={lab.id}
                ...
                <div className="flex items-start gap-4 mb-3">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 leading-snug">{field(lab, 'name')}</h3>
                </div>

                {field(lab, 'description') && (
                  <p className="text-slate-500 text-sm leading-relaxed mb-4">{field(lab, 'description')}</p>
                )}

                {lab.capacity && (
                  <div className="mt-auto pt-4 border-t border-slate-100 flex items-center text-sm text-slate-600">
                    <Users className="w-4 h-4 mr-2 text-slate-400" />
                    {t('laboratories.capacity', 'Вместимость')}: {lab.capacity} {t('laboratories.students', 'студентов')}
                  </div>
                )}
              </div>
            ))}
